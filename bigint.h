...
namespace plastic {
	...
	private:
		...
		/*
			Table[Piecewise[{{c - 48, 48 <= c <= 57}, {c - 55, 65 <= c <= 90}, {c - 87, 97 <= c <= 122}, {36, True}}], {c, 0, 255}]
			StringJoin[CharacterRange[48, 57], CharacterRange[97, 122]]
		...
		*/
		...
		static constexpr ::std::array<::std::uint8_t, 256> _fromCharTable{
		...
		static constexpr ::std::string_view _toCharTable{"0123456789abcdefghijklmnopqrstuvwxyz"};

		static constexpr ::std::array<::std::uint64_t, 37> _fromStringLogTable{
		...
		static constexpr ::std::array<::std::uint64_t, 37> _toStringLogTable{
		...
		static constexpr ::std::array<::std::size_t, 37> _chunkSizeTable{
			0, 0, 0, 40, 0, 27, 24, 22, 0, 20, 19, 18, 17, 17, 16, 16,
			0, 15, 15, 15, 14, 14, 14, 14, 13, 13, 13, 13, 13, 13, 13, 12, 0, 12, 12, 12,
			12
		};

		static constexpr ::std::array<::std::uint64_t, 37> _chunkBaseTable{
		...
		static ::std::pair<::std::uint64_t, ::std::uint8_t> _add(::std::uint64_t num1, ::std::uint64_t num2, ::std::uint8_t carry = 0) {
			::std::uint64_t sum;
			carry = _addcarry_u64(carry, num1, num2, &sum);
			return {sum, carry};
		}

		static ::std::pair<::std::uint64_t, ::std::uint8_t> _sub(::std::uint64_t num1, ::std::uint64_t num2, ::std::uint8_t borrow = 0) {
			::std::uint64_t diff;
			borrow = _subborrow_u64(borrow, num1, num2, &diff);
			return {diff, borrow};
		}
...
#else
		static ::std::pair<::std::uint64_t, ::std::uint8_t> _add(::std::uint64_t num1, ::std::uint64_t num2, ::std::uint8_t carry = 0) {
			::std::uint64_t sum{num1 + num2 + carry};
			return {sum, carry == 0 ? sum < num1 : sum <= num1};
		}

		static ::std::pair<::std::uint64_t, ::std::uint8_t> _sub(::std::uint64_t num1, ::std::uint64_t num2, ::std::uint8_t borrow = 0) {
			::std::uint64_t diff{num1 - num2 - borrow};
			return {diff, borrow == 0 ? diff > num1 : diff >= num1};
		}

		static ::std::pair<::std::uint64_t, ::std::uint64_t> _mul(::std::uint64_t num1, ::std::uint64_t num2) {
			::std::uint32_t n1l{static_cast<::std::uint32_t>(num1)};
			::std::uint32_t n1h{num1 >> 32};
			::std::uint32_t n2l{static_cast<::std::uint32_t>(num2)};
			::std::uint32_t n2h{num2 >> 32};

			::std::uint64_t t0{::std::uint64_t{n1l} *n2l};
			::std::uint64_t t1{::std::uint64_t{n1l} *n2h};
			::std::uint64_t t2{::std::uint64_t{n1h} *n2l};
			::std::uint64_t t3{::std::uint64_t{n1h} *n2h};
			t1 += t2 + (t0 >> 32);
			if (t1 < t2) {
				t3 += 0x100000000;
			}

			return {t3 + (t1 >> 32), (t1 << 32) + static_cast<::std::uint32_t>(t0)};
		}

		static ::std::pair<::std::uint64_t, ::std::uint64_t> _div(::std::uint64_t num1High, ::std::uint64_t num1Low, ::std::uint64_t num2) {
			::std::uint32_t n1l{static_cast<::std::uint32_t>(num1Low)};
			::std::uint32_t n1h{num1Low >> 32};
			::std::uint32_t n2l{static_cast<::std::uint32_t>(num2)};
			::std::uint32_t n2h{num2 >> 32};

			::std::uint64_t qh{num1High / n2h};
			::std::uint64_t rh{num1High % n2h << 32 | n1h};
			::std::uint64_t t{qh * n2l};
			if (rh < t) {
				--qh;
				rh += num2;
				if (rh >= num2 && rh < t) {
			...
			::std::uint64_t ql{rh / n2h};
			::std::uint64_t rl{rh % n2h << 32 | n1l};
			t = ql * n2l;
			if (rl < t) {
				--ql;
				rl += num2;
		...
		static ::std::uint8_t _add(data_type::const_iterator begin, data_type::const_iterator end, ::std::uint64_t num, data_type::iterator dest) {
			if (num == 0) {
				return 0;
			}
			while (begin != end) {
				::std::tie(*dest++, num) = _add(*begin++, num);
			}
			return num;
		}

		static ::std::uint8_t _add(data_type::const_iterator begin1, data_type::const_iterator end1, data_type::const_iterator begin2, data_type::const_iterator end2, data_type::iterator dest, ::std::uint8_t carry = 0) {
			while (begin2 != end2) {
				::std::tie(*dest++, carry) = _add(*begin1++, *begin2++, carry);
			}
			return _add(begin1, end1, carry, dest);
		}

		static ::std::uint8_t _sub(data_type::const_iterator begin, data_type::const_iterator end, ::std::uint64_t num, data_type::iterator dest) {
			if (num == 0) {
				return 0;
			}
			while (begin != end) {
				::std::tie(*dest++, num) = _sub(*begin++, num);
			}
			return num;
		}

		static ::std::uint8_t _sub(data_type::const_iterator begin1, data_type::const_iterator end1, data_type::const_iterator begin2, data_type::const_iterator end2, data_type::iterator dest, ::std::uint8_t borrow = 0) {
			while (begin2 != end2) {
				::std::tie(*dest++, borrow) = _sub(*begin1++, *begin2++, borrow);
			}
		...
		static ::std::uint64_t _div(data_type::const_iterator begin, data_type::const_iterator end, ::std::uint64_t num, data_type::iterator dest, ::std::uint64_t borrow = 0) {
		...
		static bigint& _add(bigint& num1, ::std::uint64_t num2) {
			::std::uint8_t carry{_add(num1._data.begin(), num1._data.end(), num2, num1._data.begin())};
			if (carry != 0) {
		...
		static bigint& _add(bigint& num1, const bigint& num2) {
			...
			::std::uint8_t carry{_add(p1->_data.begin(), p1->_data.end(), p2->_data.begin(), p2->_data.end(), num1._data.begin())};
			if (carry != 0) {
				num1._data.emplace_back(carry);
			}
			return num1;
	...
	public:
		...
		explicit bigint(::std::string_view num, unsigned base = 0) {
			...
			if (::std::has_single_bit(base)) {
				::std::size_t baseBits{_fromStringLogTable[base]};
				...
				while (begin != end) {
					::std::uint8_t digit{_fromCharTable[static_cast<::std::uint8_t>(*--end)]};
					if (digit >= base) {
						::std::abort();
					}
			...
			::std::uint64_t log{_fromStringLogTable[base]};
			::std::size_t chunkSize{_chunkSizeTable[base]};
			::std::uint64_t chunkBase{_chunkBaseTable[base]};
				...
				while (begin != end) {
					...
					if (end - begin >= chunkSize) {
						...
						while (begin != sentinel) {
							::std::uint8_t digit{_fromCharTable[static_cast<::std::uint8_t>(*begin++)]};
							if (digit >= base) {
								::std::abort();
							}
		...
		friend ::std::string to_string(const bigint& num, unsigned base = 10) {
			...
			if (::std::has_single_bit(base)) {
				::std::size_t baseBits{_toStringLogTable[base]};
				...
				while (true) {
					while (offset < 64) {
						*sEnd++ = _toCharTable[*end >> offset & mask];
						offset -= baseBits;
					}
					if (begin == end) {
						break;
					...
					offset &= 63;
					*sEnd++ = _toCharTable[high | *--end >> offset];
					offset -= baseBits;
				}
			...
			if (num._data.size() <= 35) {
				::std::uint64_t log{_toStringLogTable[base]};
				::std::size_t chunkSize{_chunkSizeTable[base]};
				::std::uint64_t chunkBase{_chunkBaseTable[base]};
