...
namespace tests {
    ...
    void test_search_tree() {
        ...
        ASSERT(tests::format(c.cbegin(), c.cend()) == "[1, 2, 3]");
        ASSERT(tests::format(c.rbegin(), c.rend()) == "[3, 2, 1]");
        ASSERT(tests::format(c.crbegin(), c.crend()) == "[3, 2, 1]");
        ...
        x.insert(2);
        ASSERT(tests::format(x) == "[1, 2, 2, 2, 2, 3]");
        x.insert(4);
        ASSERT(tests::format(x) == "[1, 2, 2, 2, 2, 3, 4]");
        x.insert(0);
        ASSERT(tests::format(x) == "[0, 1, 2, 2, 2, 2, 3, 4]");
        x.insert({ 5, 3, 1, 0, 2 });
        ASSERT(tests::format(x) == "[0, 0, 1, 1, 2, 2, 2, 2, 2, 3, 3, 4, 5]");

        x.erase(2);
        ASSERT(tests::format(x) == "[0, 0, 1, 1, 3, 3, 4, 5]");
        x.erase(1);
        ASSERT(tests::format(x) == "[0, 0, 3, 3, 4, 5]");
        x.erase(++x.begin());
        ASSERT(tests::format(x) == "[0, 3, 3, 4, 5]");
        x.erase(std::ranges::next(x.begin(), 2), std::ranges::prev(x.end(), 2));
        ASSERT(tests::format(x) == "[0, 3, 4, 5]");

        x.merge(x);
        ASSERT(tests::format(x) == "[0, 3, 4, 5]");
        x.merge(a);
        ASSERT(tests::format(x) == "[0, 0, 0, 0, 3, 4, 5]");
        ASSERT(tests::format(a) == "[]");
        x.merge(c);
        ASSERT(tests::format(x) == "[0, 0, 0, 0, 1, 2, 3, 3, 4, 5]");
